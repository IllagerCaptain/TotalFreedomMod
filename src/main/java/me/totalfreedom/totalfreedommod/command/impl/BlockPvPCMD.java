...
@CommandParameters(name = "blockpvp", description = "Toggle PVP mode for everyone or a certain player.", usage = "/<command> [[-s] <player> [reason] | list | purge | all]", aliases = "pvpblock,pvpmode")
public class BlockPvPCMD extends FreedomCommand {
    ...
    public boolean run(final CommandSender sender, final Player playerSender, final Command cmd, final String commandLabel, String[] args, final boolean senderIsConsole) {
        ...
        final FPlayer pd = plugin.playerList.getPlayer(p);
        if (pd.isPvpBlocked()) {
            ...
            if (smite) {
                SmiteCMD.smite(sender, p, reason);
            }
            plugin.punishmentList.logPunishment(new Punishment(p.getName(), FUtil.getIp(p), sender.getName(), PunishmentType.BLOCKPVP, null));
