...
@CommandParameters(name = "nickclean", description = "Essentials Interface Command - Remove illegal chatcodes from nicknames of one or all players on server.", usage = "/<command> [player]", aliases = "nc")
public class NickCleanCMD extends FreedomCommand {
    ...
    public void cleanNickname(Player player) {
        ...
        final String nickName = plugin.essentialsBridge.isEnabled() ? plugin.essentialsBridge.getNickname(playerName) : player.getDisplayName();
        ...
        if (plugin.essentialsBridge.isEnabled())
            plugin.essentialsBridge.setNickname(playerName, newNick.toString());
