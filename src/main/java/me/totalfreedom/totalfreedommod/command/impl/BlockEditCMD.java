...
@CommandParameters(name = "blockedit", description = "Restricts/unrestricts block modification abilities for everyone on the server or a certain player.", usage = "/<command> [[-s] <player> [reason] | list | purge | all]")
public class BlockEditCMD extends FreedomCommand {
    ...
    public boolean run(final CommandSender sender, final Player playerSender, final Command cmd, final String commandLabel, String[] args, final boolean senderIsConsole) {
        ...
        if (args[0].equals("list")) {
            ...
            for (Player player : server.getOnlinePlayers()) {
                final FPlayer info = plugin.playerList.getPlayer(player);
        ...
        final FPlayer pd = plugin.playerList.getPlayer(player2);
        if (pd.isEditBlocked()) {
            ...
            if (plugin.adminList.isAdmin(player2)) {
            ...
            if (smite) {
                SmiteCMD.smite(sender, player2, reason);
            }
            ...
            plugin.punishmentList.logPunishment(new Punishment(player2.getName(), FUtil.getIp(player2), sender.getName(), PunishmentType.BLOCKEDIT, null));
        }
        return true;
    }
}