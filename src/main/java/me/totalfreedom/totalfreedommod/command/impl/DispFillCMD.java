...
@CommandParameters(name = "dispfill", description = "Fill nearby dispensers with a set of items of your choice.", usage = "/<command> <radius> <comma,separated,items>")
public class DispFillCMD extends FreedomCommand {
                    ...
                    for (int zOffset = -radius; zOffset <= radius; zOffset++) {
                        ...
                        if (targetBlock.getLocation().distanceSquared(centerLocation) < (radius * radius)) {
                            if (targetBlock.getType().equals(Material.DISPENSER)) {
                                ...
                                plugin.coreProtectBridge.getCoreProtectAPI().logContainerTransaction(sender.getName(), targetBlock.getLocation());
