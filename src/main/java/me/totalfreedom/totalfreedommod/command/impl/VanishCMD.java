...
@CommandParameters(name = "vanish", description = "Vanish/unvanish yourself.", usage = "/<command> [-s[ilent]]", aliases = "v")
public class VanishCMD extends FreedomCommand {

    public boolean run(final CommandSender sender, final Player playerSender, final Command cmd, final String commandLabel, final String[] args, final boolean senderIsConsole) {
        Displayable display = plugin.rankManager.getDisplay(playerSender);
        ...
        if (plugin.adminList.isVanished(playerSender.getName())) {
            if (silent) {
                msg(ChatColor.GOLD + "Silently unvanished.");
            } else {
                msg("You have unvanished.", ChatColor.GOLD);
                FUtil.bcastMsg(plugin.rankManager.craftLoginMessage(playerSender, null));
                FUtil.bcastMsg(playerSender.getName() + " joined the game.", ChatColor.YELLOW);
                plugin.discord.messageChatChannel("**" + playerSender.getName() + " joined the server" + "**");
            }
            ...
            plugin.playerList.getData(playerSender).setTag(tag);
            FLog.info(playerSender.getName() + " is no longer vanished.");
            plugin.adminList.messageAllAdmins(ChatColor.YELLOW + sender.getName() + " has unvanished and is now visible to everyone.");
            ...
            plugin.essentialsBridge.setVanished(playerSender.getName(), false);
            ...
            if (silent) {
                ...
                plugin.discord.messageChatChannel("**" + playerSender.getName() + " left the server" + "**");
            }

            FLog.info(playerSender.getName() + " is now vanished.");
            plugin.adminList.messageAllAdmins(ChatColor.YELLOW + sender.getName() + " has vanished and is now only visible to admins.");
            ...
            plugin.essentialsBridge.setVanished(playerSender.getName(), true);
