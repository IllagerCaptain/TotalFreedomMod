...
@CommandParameters(name = "whitelist", description = "Manage the whitelist.", usage = "/<command> <on | off | list | count | add <player> | remove <player> | addall | purge>")
public class WhitelistCMD extends FreedomCommand {
    @Override
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        ...
        if (args[0].equalsIgnoreCase("purge")) {
            ...
            msg("Removed " + plugin.serverInterface.purgeWhitelist() + " players from the whitelist.");
            return true;
        }
        return false;
    }
    ...
    public List<String> getTabCompleteOptions(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            ...
            return arguments;
        } else if (args.length == 2 && plugin.adminList.isAdmin(sender)) {
    ...
    public List<String> getWhitelistedNames() {
        return Bukkit.getWhitelistedPlayers().stream().map(OfflinePlayer::getName).collect(Collectors.toList());
    }
}