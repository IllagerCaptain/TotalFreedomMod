...
@CommandParameters(name = "tag", description = "Allows you to set your own prefix.", usage = "/<command> [-s[ave]] <set <tag..> | list | gradient <hex> <hex> <tag..> | off | clear <player> | clearall>")
        ...
        if (args.length == 1) {
            switch (args[0].toLowerCase()) {
                case "list" -> {
                    ...
                    for (final Player player : server.getOnlinePlayers()) {
                        if (plugin.adminList.isVanished(player.getName()) && !plugin.adminList.isAdmin(sender)) {
                            continue;
                        }
                ...
                case "off" -> {
                    if (senderIsConsole) {
                        ...
                        plugin.playerList.getPlayer(playerSender).setTag(null);
            ...
            switch (args[0].toLowerCase()) {
                case "clear": {
                    ...
                    plugin.playerList.getPlayer(player).setTag(null);
                    if (save) {
                        save(player, null);
                    }
                ...
                case "set": {
                    ...
                    int tagLimit = (plugin.adminList.isAdmin(sender) ? 30 : 20);
                    ...
                    plugin.playerList.getPlayer(playerSender).setTag(outputTag);
