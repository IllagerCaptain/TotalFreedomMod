...
@CommandParameters(name = "panel", description = "Manage your Pterodactyl panel account", usage = "/<command> <create | delete>")
public class PanelCMD extends FreedomCommand {

    @Override
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {

        if (!plugin.pterodactyl.isEnabled()) {
        ...
        if (args[0].equalsIgnoreCase("create")) {
            ...
            String id = plugin.pterodactyl.createAccount(username, password);
            if (Strings.isNullOrEmpty(id)) {
                msg("Failed to create your Pterodactyl account.", ChatColor.RED);
                return true;
            }

            plugin.pterodactyl.addAccountToServer(id);

            admin.setPteroID(id);
            plugin.adminList.save(admin);
            plugin.adminList.updateTables();

            plugin.discord.sendPteroInfo(playerData, username, password);
            msg("Successfully created your Pterodactyl account. Check your DMs from " + plugin.discord.formatBotTag() + " on discord to get your credentials.", ChatColor.GREEN);
            return true;
            ...
            boolean deleted = plugin.pterodactyl.deleteAccount(admin.getPteroID());
    ...
    public List<String> getTabCompleteOptions(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1 && plugin.adminList.isSeniorAdmin(sender)) {
