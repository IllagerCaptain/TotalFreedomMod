...
@CommandParameters(name = "nickfilter", description = "NickFilter: Prefix any command with this command to replace nicknames in that command with real names. Nicknames should be prefixed with a !.",
...
public class NickFilterCMD extends FreedomCommand {
    ...
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        ...
        if (args.length >= 1) {
            for (String arg : args) {
                ...
                if (matcher.find()) {
                    ...
                    if (player == null || plugin.adminList.isVanished(player.getName()) && !plugin.adminList.isAdmin(sender)) {
        ...
        if (plugin.commandBlocker.isCommandBlocked(newCommand, sender)) {
