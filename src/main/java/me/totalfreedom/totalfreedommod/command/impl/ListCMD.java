...
@CommandParameters(name = "list", description = "Lists the real names of all online players.", usage = "/<command> [-s | -i | -f | -v]", aliases = "who,lsit")
public class ListCMD extends FreedomCommand {
    ...
    public boolean run(final CommandSender sender, final Player playerSender, final Command cmd, final String commandLabel, final String[] args, final boolean senderIsConsole) {
        ...
        if (listFilter == ListFilter.TELNET_SESSIONS && plugin.adminList.isAdmin(sender)) {
            List<Admin> connectedAdmins = plugin.bukkitTelnetBridge.getConnectedAdmins();
            ...
            for (Player p : server.getOnlinePlayers()) {
                if (listFilter == ListFilter.ADMINS && !plugin.adminList.isAdmin(p)) {
                    continue;
                }
                if (listFilter == ListFilter.ADMINS && plugin.adminList.isVanished(p.getName())) {
                    continue;
                }
                if (listFilter == ListFilter.VANISHED_ADMINS && !plugin.adminList.isVanished(p.getName())) {
                    continue;
                }
                if (listFilter == ListFilter.IMPOSTORS && !plugin.adminList.isAdminImpostor(p)) {
                    continue;
                }
                if (listFilter == ListFilter.FAMOUS_PLAYERS && !ConfigEntry.FAMOUS_PLAYERS.getList().contains(p.getName().toLowerCase())) {
                    continue;
                }
                if (listFilter == ListFilter.PLAYERS && plugin.adminList.isVanished(p.getName())) {
                    continue;
                }

                final Displayable display = plugin.rankManager.getDisplay(p);
