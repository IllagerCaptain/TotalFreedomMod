...
@CommandParameters(name = "entitywipe", description = "Remove various server entities that may cause lag, such as dropped items, minecarts, and boats.", usage = "/<command> [name | -a]", aliases = "ew,rd, ewipe, entityw")
public class EntityWipeCMD extends FreedomCommand {
    ...
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        ...
        int count;
        if (type != null) {
            count = plugin.entityWiper.wipeEntities(type);
        } else {
            count = plugin.entityWiper.wipeEntities(bypassBlacklist);
        }
        if (count == 1) {
