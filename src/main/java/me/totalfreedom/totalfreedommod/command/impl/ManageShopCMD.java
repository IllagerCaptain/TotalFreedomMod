...
@CommandParameters(name = "manageshop", description = "Manage the shop", usage = "/<command> <coins: <add | set | remove> <amount> <player | all> | items: <give | take> <item> <player>", aliases = "ms")
public class ManageShopCMD extends FreedomCommand {
    @Override
                ...
                case "add" -> {
                    try {
                        ...
                        if (!args[3].equals("all")) {
                            PlayerData playerData = plugin.playerList.getData(args[3]);
                            if (playerData == null) {
