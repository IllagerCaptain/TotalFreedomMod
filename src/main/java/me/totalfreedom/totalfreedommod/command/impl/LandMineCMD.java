...
@CommandParameters(name = "landmine", description = "Set a landmine trap.", usage = "/<command>")
public class LandMineCMD extends FreedomCommand {
    ...
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        ...
        if (args.length >= 1) {
            if ("list".equals(args[0])) {
                final Iterator<Landmine> landmines = plugin.landMiner.getLandmines().iterator();
        ...
        plugin.landMiner.add(new Landmine(landmine.getLocation(), playerSender, radius));
