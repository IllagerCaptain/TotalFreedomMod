...
@CommandParameters(name = "ride", description = "Ride on the top of the specified player.", usage = "/<command> <playername | mode <normal | off | ask>>")
public class RideCMD extends FreedomCommand {
        ...
        if (args.length >= 2) {
            if (args[0].equalsIgnoreCase("mode")) {
                if (args[1].equalsIgnoreCase("normal") || args[1].equalsIgnoreCase("off") || args[1].equalsIgnoreCase("ask")) {
                    PlayerData playerDataSender = plugin.playerList.getData(playerSender);
                    playerDataSender.setRideMode(args[1].toLowerCase());
                    plugin.playerList.save(playerDataSender);
