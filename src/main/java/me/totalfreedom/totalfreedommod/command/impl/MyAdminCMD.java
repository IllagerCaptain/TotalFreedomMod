...
@CommandParameters(name = "myadmin", description = "Manage your admin entry.", usage = "/<command> [-o <admin name>] <clearips | clearip <ip> | setscformat <format> | clearscformat> | syncroles>")
public class MyAdminCMD extends FreedomCommand {
    @Override
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        ...
        switch (args[0]) {
            case "clearips": {
                ...
                plugin.adminList.save(target);
                plugin.adminList.updateTables();
                plugin.playerList.syncIps(target);
            ...
            case "syncroles": {
                if (plugin.discord.enabled) {
                    ...
                    PlayerData playerData = plugin.playerList.getData(target.getName());
            ...
            if (args[0].equals("-o")) {
                ...
                if (doubleArguments.contains(args[0])) {
                    if (args[0].equals("clearip")) {
                        List<String> ips = plugin.adminList.getAdmin(sender).getIps();
