...
@CommandParameters(name = "invsee", description = "Look into another player's inventory, or optionally take items out.", usage = "/<command> <player> [offhand | armor]", aliases = "inv,insee")
public class InvSeeCMD extends FreedomCommand {
    @Override
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        ...
        if (plugin.adminList.isAdmin(player) && !plugin.adminList.isAdmin(playerSender)) {
        ...
        if (!plugin.adminList.isAdmin(player)) {
            FPlayer fPlayer = plugin.playerList.getPlayer(playerSender);
