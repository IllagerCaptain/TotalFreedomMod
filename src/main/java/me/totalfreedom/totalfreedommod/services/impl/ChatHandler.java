...
public class ChatHandler extends AbstractService
{
    @Override
    public void onStart()
    {
    ...
    private void handleChatEvent(AsyncPlayerChatEvent event)
    {
        ...
        if (ConfigEntry.SHOP_ENABLED.getBoolean() && ConfigEntry.SHOP_REACTIONS_ENABLED.getBoolean() && !plugin.shop.reactionString.isEmpty() && message.equals(plugin.shop.reactionString))
        {
            event.setCancelled(true);
            PlayerData data = plugin.playerList.getData(player);
            data.setCoins(data.getCoins() + plugin.shop.coinsPerReactionWin);
            plugin.playerList.save(data);
            plugin.shop.endReaction(player.getName());
            player.sendMessage(ChatColor.GREEN + "You have been given " + ChatColor.GOLD + plugin.shop.coinsPerReactionWin + ChatColor.GREEN + " coins!");
            return;
        }

        if (!ConfigEntry.TOGGLE_CHAT.getBoolean() && !plugin.adminList.isAdmin(player))
        {
            event.setCancelled(true);
        ...
        final FPlayer fPlayer = plugin.playerList.getPlayerSync(player);
        ...
        // Check for mentions
        boolean mentionEveryone = ChatColor.stripColor(message).toLowerCase().contains("@everyone") && plugin.adminList.isAdmin(player);
        ...
        // Send to discord
        if (!ConfigEntry.ADMIN_ONLY_MODE.getBoolean() && !Bukkit.hasWhitelist() && !plugin.playerList.getPlayer(player).isMuted() && !plugin.tfGuildsBridge.inGuildChat(player))
        {
            plugin.discord.messageChatChannel(player.getName() + " \u00BB " + ChatColor.stripColor(message));
        }
    }
    ...
    {
        Displayable display = plugin.rankManager.getDisplay(sender);
        FLog.info("[ADMIN] " + sender.getName() + " " + display.getTag() + ": " + message, true);
        plugin.discord.messageAdminChatChannel(sender.getName() + " \u00BB " + message);

        server.getOnlinePlayers().stream().filter(player -> plugin.adminList.isAdmin(player)).forEach(player ->
        {
