...
public class LoginProcess extends AbstractService
{
    ...
    public void onPlayerPreLogin(AsyncPlayerPreLoginEvent event)
    {
        ...
        final boolean isAdmin = plugin.adminList.getEntryByIp(ip) != null;
    ...
    public void onPlayerLogin(PlayerLoginEvent event)
    {
        ...
        if (plugin.adminList.getEntryByIp(ip) != null) // Check if player is admin
        {
            // Force-allow log in
            event.allow();
            ...
            if (count >= server.getMaxPlayers())
            {
                for (Player onlinePlayer : server.getOnlinePlayers())
                {
                    if (!plugin.adminList.isAdmin(onlinePlayer))
                    {
        ...
        // Whitelist
        if (plugin.serverInterface.isWhitelisted())
        {
            if (!plugin.serverInterface.isWhitelisted(player))
            {
    ...
    public void onPlayerJoin(PlayerJoinEvent event)
    {
        ...
        final PlayerData playerData = plugin.playerList.getData(player);
        ...
        if (!plugin.adminList.isAdmin(player))
        {
            ...
            if (noteCount != 0)
            {
                ...
                plugin.adminList.messageAllAdmins(ChatColor.GOLD + noteMessage);
                plugin.adminList.messageAllAdmins(ChatColor.GOLD + "Do " + ChatColor.YELLOW + "/notes " + player.getName() + " list" + ChatColor.GOLD + " to view them.");
            }
        }
